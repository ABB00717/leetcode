...
class Solution {
   public:
    ...
    TreeNode* lcaDeepestLeaves(TreeNode* root) {
        ...
        std::unordered_map<TreeNode*, TreeNode*> parentMap;  // Map to store parent of each node.
        std::queue<TreeNode*> q;  // Queue for level-order traversal.
        q.push(root);
        std::vector<TreeNode*> lastLevel;  // Vector to store nodes of the last level.
        ...
        while (!q.empty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                q.pop();
                lastLevel.push_back(node);  // Add current node to the last level vector.
        ...
        while (lastLevel.size() > 1) {
            std::unordered_set<TreeNode*> parents;  // Set to store unique parents.
            for (auto node : lastLevel) {
                parents.insert(parentMap[node]);  // Add parent of each node to the set.
            }
            lastLevel.assign(parents.begin(), parents.end());  // Update last level with parents.
        }

        return lastLevel[0];  // The remaining node is the LCA of the deepest leaves.
    }
};
